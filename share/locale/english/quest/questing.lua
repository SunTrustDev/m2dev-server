...
--[[
    ...
    @descr  Splittet einen String in eine Tabelle.
--]]
...
--[[
    ...
    @author Mijago
    @needs  split
    @descr Mysql-Funktion der neuesten Generation.
--]]
mysql_query = function(query)
    ...
    for av in io.open(fi, 'r'):lines() do table.insert(t, split(av, '\t')) end
    os.remove(fi);
    for i = 2, #t do
...
--[[
    ...
    @author Mijago
    @needs  split
    @descr Die Alte Version der MySQL-Query-Funktion.
--]]
local ql = {
    ["user"] = "root",
    ["pass"] = "",
...
function mysql_query_old(query, user, pass, db, ip)
    local pre = ''
    if query == '' or query == nil then error("Query muss gesetzt sein!") end
    ...
    math.randomseed(os.time());
    local rand = math.random(0, 10 ^ 7) -- Erstellen der Pfadvariable
    local path = 'data/mysql_output_' .. os.time() .. '_' .. rand .. '_' ..
                     pc.get_vid()
    os.execute("mysql " .. pre .. " --e=\"" .. query .. "\" > " .. path) -- Laden und Auflisten der Dateiinhalte
    local fi, q = io.open(path, "r"), {["l"] = {}, ["out"] = {}}
    if fi == nil then return "ERROR" end
    ...
    local ix = 0
    for i, l in ipairs(q.l) do
        if i > 1 then
            for i2, l2 in pairs(l) do
                if q.out[q.l[1][i2]] == nil then
                    q.out[q.l[1][i2]] = {}
                end
                local c = tonumber(l2)
...
--[[
    @name   define
    @author Mijago
    @descr Gives the possibility to define global variables. Functions can also be used! These are then EXECUTED and returned!
--]]
_G.__data = {}
...
--[[
    @name   duration
    @author Mijago
    @descr Returns the remaining time as a string.
--]]
function duration(ipe)
    ...
    for x, c in {
        {s, "Sek."}, {m, "Min."}, {h, "Std."}, {d, "Tage", "Tag"},
        {mo, "Monate", "Monat"}, {y, "Jahre", "Jahr"}
    } do
        if (c[1] or 0) > 0 then
...
--[[
    @name   is_number
    @author Mijago
    @descr Checks if a variable is a number.
--]]
...
--[[
    @name   is_string
    @author Mijago
    @descr Checks if a variable is a string.
--]]
...
--[[
    @name   is_table
    @author Mijago
    @descr Checks if a variable is a table.
--]]
...
--[[
    @name   in_table
    @author Mijago
    @descr Checks if a variable is in a table.
    Call: in_table(var,table)
--]]
...
--[[
    @name   numlen
    @author Mijago
    @descr Returns the number of digits of a number.
--]]
function numlen(i)
    local i, x = i or 0, 0
...
--[[
    @name   string.reverse
    @author Mijago
    @descr Reverses a string.
--]]
function string.reverse(str)
    local se = ''
    for i = 1, string.len(str) do se = string.sub(str, i, i) .. se end
    return se
end
...
--[[
    ...
    @descr Formats long numbers with dots.
--]]
...
--[[
    @name   numtomoney
    @author Mijago
    @descr Formats numbers with dots.
...
--[[
    @name   math.minmax
    @author Mijago
    @descr Allows the specification of min and max at once.
--]]
...
--[[
    @name   n_input
    @author Mijago
    @descr For inputs only for numbers.
    The number is ALWAYS positive. If it is not valid, it is 0.
--]]
function n_input() return math.abs(tonumber(input()) or 0) end

--[[
    @name   long_input
    @author Mijago
    @descr Allows the use of longer inputs.
--]]
function long_input()
    local str, t = "", input()
    while t ~= "" do
...
--[[
    ...
    @needs  split
    @descr Like Select:
    A table or a string list is split into pages.
...
function select2(tab, ...)
    ...
    local max = tab[1];
    table.remove(tab, 1)
    local tablen, outputstr, outputcount, nextc, incit = #tab, "", 0, 0, 0
    table.foreach(tab, function(i, l)
        ...
        elseif outputcount == max and tablen > outputcount + incit then
            if tablen ~= outputcount + incit + 1 then
                outputstr =
                    outputstr .. ',"' .. l .. '","N�chste Seite") + ' .. incit ..
                        ' '
                if nextc > 0 then outputstr = outputstr .. 'end ' end
                outputstr = outputstr .. '; if sel == ' .. (incit + max + 1) ..
                                ' then ' -- Anfangen der neuen Abfrage
...
--[[
    ...
    @descr Like Select2:
    A table or a string list is split into pages.
    Next, Back and Cancel (-1) buttons.
--]]
function select3(...)
    arg.n = nil
    local tp, max = arg, 5
    ...
    local function copy_tab(t)
        local p = {}
        for i = 1, #t do p[i] = t[i] end
        return p
    end
    local pe = {}
    for i = 1, #px do pe[i] = copy_tab(px[i]) end
    local function init(i, ip)
        pe[i] = copy_tab(px[i])
        local next, back, exit = 0, 0, 0
        if i < #pe and #pe ~= 1 then
            table.insert(pe[i], table.getn(pe[i]) + 1,
                         'Weiter zu Seite ' .. (i + 1));
            next = table.getn(pe[i])
        end
        if i > 1 then
            table.insert(pe[i], table.getn(pe[i]) + 1,
                         'Zur�ck zu Seite ' .. (i - 1));
            back = table.getn(pe[i])
        end
        table.insert(pe[i], table.getn(pe[i]) + 1, 'Abbruch');
        exit = table.getn(pe[i])
        if #pe > 1 then say('Seite ' .. i .. ' von ' .. #pe) end
        local e = select_table(pe[i])
        if e == next then
            return init(i + 1, ip + max)
        elseif e == back then
            return init(i - 1, ip - max)
        elseif e == exit then
            return -1
...
--[[
    ...
    @author Mijago
    @descr Like Notice, but with the player's name before.
--]]
function note(text) notice_all(pc.get_name() .. ': ' .. text) end

--[[
    @name   Zeitrechnungen
    @author Mijago
    @descr Functions to convert time.
--]]
zt = zt or {}
zt.d_j = function(d) return d / 365 end
zt.d_mo = function(d) return d / 12 end
zt.d_h = function(d) return d * 24 end
zt.d_m = function(d) return d * 24 * 60 end
zt.d_s = function(d) return d * 24 * 60 * 60 end
zt.d_hs = function(d) return d * 24 * 60 * 60 * 100 end
zt.d_ms = function(d) return d * 24 * 60 * 60 * 1000 end
--- Hours
zt.h_j = function(h) return h / 24 / 365 end
zt.h_mo = function(h) return h / 24 / 12 end
zt.h_d = function(h) return h / 24 end
zt.h_m = function(h) return h * 60 end
zt.h_s = function(h) return h * 60 * 60 end
zt.h_hs = function(h) return h * 60 * 60 * 100 end
zt.h_ms = function(h) return h * 60 * 60 * 1000 end
--- Minutes
zt.m_j = function(m) return m / 60 / 24 / 365 end
zt.m_mo = function(m) return m / 60 / 24 / 12 end
zt.m_d = function(m) return m / 60 / 24 end
zt.m_h = function(m) return m / 60 end
zt.m_s = function(m) return m * 60 end
zt.m_hs = function(m) return m * 60 * 100 end
zt.m_ms = function(m) return m * 60 * 1000 end
--- Seconds
zt.s_j = function(s) return s / 60 / 60 / 24 / 365 end
zt.s_mo = function(s) return s / 60 / 60 / 24 / 12 end
zt.s_d = function(s) return s / 60 / 60 / 24 end
zt.s_h = function(s) return s / 60 / 60 end
zt.s_m = function(s) return s / 60 end
zt.s_hs = function(s) return s * 100 end
zt.s_ms = function(s) return s * 1000 end

--[[
    @name   Autoumbruch in Say
    @author Mijago
    @descr Like Say, but with automatic line breaks.
--]]
...
--[[
    @name   mysql_escape
    @author Mijago
    @descr Like mysql_real_escape_string in PHP;
    Helps to prevent SQLi.
--]]
...
--[[
    ...
    @author Mijago; Idee von Benhero
    @needs  mysql_query
    @descr Like account.set_pw in PHP;
    Allows to change the user's password.
    The account can be omitted, as an account name or as an account ID.
--]]
account = account or {}
function account.set_pw(pw, ac)
    if pw == nil then error("Error... Password must be set!") end
    local ac = ac or pc.get_account_id()
    if type(ac) == "string" then
        mysql_query(
            "UPDATE player.player,account.account SET account.password = password(" ..
                string.format('%q', pw) ..
                ") WHERE account.id = player.account_id and player.name = '" ..
                ac .. "' LIMIT 1")
    elseif type(ac) == "number" then
        mysql_query("UPDATE account.account SET account.password = password(" ..
                        string.format('%q', pw) .. ") WHERE account.id = " .. ac)
    end
end
...
--[[
    ...
    @author Mijago
    @descr Checks for free inventory places for items of size X (height).
--]]
function pc.check_inventory_place(size)
    if size <= 0 or size > 3 then return -1 end
    function check(c)
        for i = 0, size - 1 do
            item.select_cell(e[c + (5 * i)])
            if item.get_id() ~= 0 then return false end
        end
        return true
    end
    for i = 0, 89 do if check(i) then return i end end
    return -1
end
...
--[[
    @name   do_for_other
    @author Mijago
    @descr Executes a string as Lua commands for another user.
--]]
...
--[[
    @name   local_pc_setqf
    @author Mijago
    @descr Sets the quest flag of another player.
--]]
function local_pc_setqf(name, qf, wert) -- For the current quest
...
--[[
    @name   pc.trans
    @author Mijago
    @descr Warps player B to player A.
    Player a = pc.
--]]
function pc.trans(vid)
    ...
    elseif type(vid) == "string" then
        ...
        if vid == 0 then error "Spieler nicht gefunden" end
    end
...
--[[
    @name   pc.warp_to
    @author Mijago
    @descr Warps player A to player B.
    Player a = pc.
--]]
...
--[[
    @name   local_warp_pc
    @author Mijago
    @descr Warps another player locally.
--]]
function local_pc_warp(name, x, y, mid)
    local target = find_pc_by_name(name)
    local t = pc.select(target)
    if mid == nil then mid = pc.get_map_index() end
...
--[[
    @name   download
    @author Mijago
    @descr Downloads a file into the Data folder.
--]]
...
--[[
    @name   dot
    @author Mijago
    @descr Executes everything between $ and $ in a string.
--]]
function dot(x)
...
--[[
    @name   dostr
    @author Mijago
    @descr Executes a string as Lua commands.
--]]
function dostr(str) assert(loadstring(str))() end

--[[
    @name   wartungsmodus
    @author Mijago
    @needs  mysql_query
    @descr Sets all accounts (except GM accounts) to a "Maintenance mode" and back.
--]]
function wartungsmodus(v)
    if v == 1 or v == true then
        mysql_query(
            "UPDATE account.account SET account.status = 'SHUTDOWN' WHERE status = 'OK' and account.login NOT IN (SELECT mAccount FROM common.gmlist);")
    else
        mysql_query(
...
--[[
    @name   create_folder
    @author Mijago 
    @descr Creates a folder, even with subfolders.
...
--[[
    @name   INI-Parser
    @author Mijago
    @descr A new parser for INI files.
--]]
ini = {
    open = function(path)
        return setmetatable({path = path or ""}, {__index = ini})
    end,
...
--[[
    @name   csay
    @author Mijago
    @descr Like the old col-commands, but sends itself.
    Instead of say(col.red('bla')), csay.red('bla') is enough.
--]]
csay = setmetatable({
    __d = {
    ...
    __index = function(tab, idx)
        ...
        return function(x)
            say('[COLOR r;' .. (color[1] / 255) .. '|g;' .. (color[2] / 255) ..
                    '|b;' .. (color[3] / 255) .. ']' .. x .. '[/COLOR]')
        end
    end
})
...
--[[
    @name   Farbcodes
    @author Mijago
    @descr Color codes for Say.
--]]
col = col or {}
col.list = {
    {'lightcoral', 240, 128, 128}, {'rosybrown', 188, 143, 143},
    {'indianred', 205, 92, 92}, {'red', 255, 0, 0}, {'firebrick', 178, 34, 34},
    {'brown', 165, 42, 42}, {'darkred', 139, 0, 0}, {'maroon', 128, 0, 0},
    {'mistyrose', 255, 228, 225}, {'salmon', 250, 128, 114},
    {'tomato', 255, 99, 71}, {'darksalmon', 233, 150, 122},
    {'coral', 255, 127, 80}, {'orangered', 255, 69, 0},
    {'lightsalmon', 255, 160, 122}, {'sienna', 160, 82, 45},
    {'seashell', 255, 245, 238}, {'chocolate', 210, 105, 30},
    {'saddlebrown', 139, 69, 19}, {'sandybrown', 244, 164, 96},
    {'peachpuff', 255, 218, 185}, {'peru', 205, 133, 63},
    {'linen', 250, 240, 230}, {'bisque', 255, 228, 196},
    {'darkorange', 255, 140, 0}, {'burlywood', 222, 184, 135},
    {'antiquewhite', 250, 235, 215}, {'tan', 210, 180, 140},
    {'navajowhite', 255, 222, 173}, {'blanchedalmond', 255, 235, 205},
    {'papayawhip', 255, 239, 213}, {'moccasin', 255, 228, 181},
    {'orange', 255, 165, 0}, {'wheat', 245, 222, 179},
    {'oldlace', 253, 245, 230}, {'floralwhite', 255, 250, 240},
    {'darkgoldenrod', 184, 134, 11}, {'goldenrod', 218, 165, 32},
    {'cornsilk', 255, 248, 220}, {'gold', 255, 215, 0},
    {'lemonchiffon', 255, 250, 205}, {'khaki', 240, 230, 140},
    {'palegoldenrod', 238, 232, 170}, {'darkkhaki', 189, 183, 107},
    {'ivory', 255, 255, 240}, {'lightyellow', 255, 255, 224},
    {'beige', 245, 245, 220}, {'lightgoldenrodyellow', 250, 250, 210},
    {'yellow', 255, 255, 0}, {'olive', 128, 128, 0},
    {'olivedrab', 107, 142, 35}, {'yellowgreen', 154, 205, 50},
    {'darkolivegreen', 85, 107, 47}, {'greenyellow', 173, 255, 47},
    {'chartreuse', 127, 255, 0}, {'lawngreen', 124, 252, 0},
    {'darkseagreen', 143, 188, 139}, {'honeydew', 240, 255, 240},
    ...
    {'mintcream', 245, 255, 250}, {'mediumspringgreen', 0, 250, 154},
    {'mediumaquamarine', 102, 205, 170}, {'aquamarine', 127, 255, 212},
    {'turquoise', 64, 224, 208}, {'lightseagreen', 32, 178, 170},
    {'mediumturquoise', 72, 209, 204}, {'azure', 240, 255, 255},
    {'lightcyan', 224, 255, 255}, {'paleturquoise', 175, 238, 238},
    {'aqua', 0, 255, 255}, {'cyan', 0, 255, 255}, {'darkcyan', 0, 139, 139},
    {'teal', 0, 128, 128}, {'darkslategray', 47, 79, 79},
    {'darkturquoise', 0, 206, 209}, {'cadetblue', 95, 158, 160},
    {'powderblue', 176, 224, 230}, {'lightblue', 173, 216, 230},
    {'deepskyblue', 0, 191, 255}, {'skyblue', 135, 206, 235},
    {'lightskyblue', 135, 206, 250}, {'steelblue', 70, 130, 180},
    {'aliceblue', 240, 248, 255}, {'dodgerblue', 30, 144, 255},
    {'lightslategray', 119, 136, 153}, {'slategray', 112, 128, 144},
    {'lightsteelblue', 176, 196, 222}, {'cornflowerblue', 100, 149, 237},
    {'royalblue', 65, 105, 225}, {'ghostwhite', 248, 248, 255},
    {'lavender', 230, 230, 250}, {'blue', 0, 0, 255}, {'mediumblue', 0, 0, 205},
    {'darkblue', 0, 0, 139}, {'midnightblue', 25, 25, 112}, {'navy', 0, 0, 128},
    {'slateblue', 106, 90, 205}, {'darkslateblue', 72, 61, 139},
    {'mediumslateblue', 123, 104, 238}, {'mediumpurple', 147, 112, 219},
    {'blueviolet', 138, 43, 226}, {'indigo', 75, 0, 130},
    {'darkorchid', 153, 50, 204}, {'darkviolet', 148, 0, 211},
    {'mediumorchid', 186, 85, 211}, {'thistle', 216, 191, 216},
    {'plum', 221, 160, 221}, {'violet', 238, 130, 238},
    {'fuchsia', 255, 0, 255}, {'magenta', 255, 0, 255},
    {'darkmagenta', 139, 0, 139}, {'purple', 128, 0, 128},
    {'orchid', 218, 112, 214}, {'mediumvioletred', 199, 21, 133},
    {'deeppink', 255, 20, 147}, {'hotpink', 255, 105, 180},
    {'lavenderblush', 255, 240, 245}, {'palevioletred', 219, 112, 147},
    {'crimson', 220, 20, 60}, {'pink', 255, 192, 203},
    {'lightpink', 255, 182, 193}, {'white', 255, 255, 255},
    {'snow', 255, 250, 250}, {'whitesmoke', 245, 245, 245},
    {'gainsboro', 220, 220, 220}, {'lightgray', 211, 211, 211},
    {'silver', 192, 192, 192}, {'darkgray', 169, 169, 169},
    {'gray', 128, 128, 128}, {'dimgray', 105, 105, 105}, {'black', 0, 0, 0},
    ...
    {'darkseagreen', 143, 188, 139}, {'darkslateblue', 72, 61, 139},
...
table.foreachi(col.list, function(a, b)
    col[b[1]] = function(text)
        return
            "[COLOR r;" .. (b[2] / 255.0) .. "|g;" .. (b[3] / 255.0) .. "|b;" .. (b[4] / 255.0) .. "]" .. text .. '[/COLOR]'
    end
end)
