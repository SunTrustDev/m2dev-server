...
q.set_clock = function(name, value)
    q.set_clock_name(name)
    q.set_clock_value(value)
end
q.set_counter = function(name, value)
    q.set_counter_name(name)
    q.set_counter_value(value)
end
...
newline = "[ENTER]"
function color256(r, g, b)
    return "[COLOR r;" .. (r / 255.0) .. "|g;" .. (g / 255.0) .. "|b;" ..
               (b / 255.0) .. "]"
end
...
-- Add a minimap signal.
function addmapsignal(x, y)
    raw_script("[ADDMAPSIGNAL x;" .. x .. "|y;" .. y .. "]")
end

-- Clear the minimap signal.
function clearmapsignal() raw_script("[CLEARMAPSIGNAL]") end

-- Set the background image.
function setbgimage(src)
    raw_script("[BGIMAGE src;")
    raw_script(src)
    raw_script("]")
end
...
function send_letter_ex(name, icon_type, icon_name)
    make_quest_button_ex(name, icon_type, icon_name)
    set_skin(NOWINDOW)
    q.set_title(name)
    q.start()
end
...
function say_reward(name)
    say(color256(255, 200, 200) .. name .. color256(196, 196, 196))
end
function say_pc_name() say(pc.get_name() .. ":") end
function say_size(width, height)
    say("[WINDOW_SIZE width;" .. width .. "|height;" .. height .. "]")
end
...
function say_item(name, vnum, desc)
    say("[INSERT_IMAGE image_type;item|idx;" .. vnum .. "|title;" .. name ..
            "|desc;" .. desc .. "|index;" .. 0 .. "|total;" .. 1 .. "]")
end
function say_show_item(vnum)
    say("[INSERT_IMAGE image_type;item|idx;" .. vnum .. "|index;" .. 0 ..
            "|total;" .. 1 .. "]")
end
function say_item_vnum(vnum) say_item(item_name(vnum), vnum, "") end
function say_item_vnum_inline(vnum, index, total)
    if index >= total then return end
    if total > 3 then return end
    raw_script("[INSERT_IMAGE image_type;item|idx;" .. vnum .. "|title;" ..
                   item_name(vnum) .. "|desc;" .. "" .. "|index;" .. index ..
                   "|total;" .. total .. "]")
end
...
function pc_has_even_id() return math.mod(pc.get_player_id(), 2) == 0 end

function pc_get_account_id() return math.mod(pc.get_account_id(), 2) ~= 0 end

village_map = {{1, 3}, {21, 23}, {41, 43}}
...
function pc_find_skill_teacher_vid(sub_job)
    local vnum = npc_get_skill_teacher_race(pc.get_empire(), pc.get_job(),
                                            sub_job)
...
function time_min_to_sec(value) return 60 * value end

function time_hour_to_sec(value) return 3600 * value end

function next_time_set(value, test_value)
    local nextTime = get_time() + value
    if is_test_server() then nextTime = get_time() + test_value end
...
function table_get_random_item(self) return self[number(1, #self)] end

function table_is_in(self, test)
    for i = 1, #self do if self[i] == test then return true end end
    return false
end

function giveup_quest_menu(title)
    local s = select("Cancel", "Cancel")
    if 2 == s then
        say(title .. " Quest cancelled")
        say("Are you sure you want to cancel the quest?")
        local s = select("Yes, continue quest", "No, cancel quest")
        if 1 == s then
            say(title .. " Quest cancelled")
            restart_quest()
        end
...
function test_chat(log) if is_test_server() then chat(log) end end
...
function show_horse_menu()
    if horse.is_mine() then
        ...
        if horse.is_dead() then
            s = select(locale.horse_menu.revive, locale.horse_menu.ride,
                       locale.horse_menu.unsummon, locale.horse_menu.close)
        else
            s = select(locale.horse_menu.feed, locale.horse_menu.ride,
                       locale.horse_menu.unsummon, locale.horse_menu.close)
        end
        ...
        if s == 1 then
            if horse.is_dead() then
                ...
                if pc.countitem(food) > 0 then
                    ...
                    say(locale.need_item_prefix .. item_name(food) ..
                            locale.need_item_postfix);
                end
            end
        elseif s == 2 then
            horse.ride()
...
npc_index_table = {['race'] = npc.getrace, ['empire'] = npc.get_empire}
...
function select_table(table) return q.yield('select', table) end

-- coroutine�� �̿��� ���� ���� ��ٸ���
function wait() q.yield('wait') end

function input() return q.yield('input') end

function confirm(vid, msg, timeout) return q.yield('confirm', vid, msg, timeout) end
...
special.fortune_telling = {
    ...
    {499, 0, 10, 10, 0}, -- 5
    {2500, 0, 5, 5, 0}, -- 1
    {5000, 0, 0, 0, 0}, {1500, 0, -5, -5, 20000}, {499, 0, -10, -10, 20000},
    {1, 0, -20, -20, 20000}
}

special.questscroll_reward = {
    {1, 1500, 3000, 30027, 0, 0}, {2, 1500, 3000, 30028, 0, 0},
    {3, 1000, 2000, 30034, 30018, 0}, {4, 1000, 2000, 30034, 30011, 0},
    {5, 1000, 2000, 30011, 30034, 0}, {6, 1000, 2000, 27400, 0, 0},
    {7, 2000, 4000, 30023, 30003, 0}, {8, 2000, 4000, 30005, 30033, 0},
    {9, 2000, 8000, 30033, 30005, 0}, {10, 4000, 8000, 30021, 30033, 30045},
    ...
    {15, 5000, 15000, 30091, 30017, 30018}, {16, 3500, 6500, 30021, 30033, 0},
    {17, 4000, 9000, 30051, 30033, 0}, {18, 4500, 10000, 30056, 30057, 30058},
    {19, 4500, 10000, 30059, 30058, 30041}, {20, 5000, 15000, 0, 0, 0}
}
...
special.skill_reset_cost = {
    2000, 2000, 2000, 2000, 2000, 2000, 4000, 6000, 8000, 10000, 14000, 18000,
    22000, 28000, 34000, 41000, 50000, 59000, 70000, 90000, 101000, 109000,
    114000, 120000, 131000, 141000, 157000, 176000, 188000, 200000, 225000,
    270000, 314000, 348000, 393000, 427000, 470000, 504000, 554000, 600000,
    758000, 936000, 1103000, 1276000, 1407000, 1568000, 1704000, 1860000,
    2080000, 2300000, 2700000, 3100000, 3500000, 3900000, 4300000, 4800000,
    5300000, 5800000, 6400000, 7000000, 8000000, 9000000, 10000000, 11000000,
...
special.levelup_reward_gold21 = {
    {10000, 20}, {20000, 50}, {40000, 25}, {80000, 3}, {100000, 2}
}
special.levelup_reward_gold31 = {
    {20000, 20}, {40000, 40}, {60000, 25}, {80000, 10}, {100000, 5}
}
special.levelup_reward_gold41 = {
    {40000, 20}, {60000, 40}, {80000, 25}, {100000, 10}, {150000, 5}
}
special.levelup_reward_gold51 = {
    {60000, 20}, {80000, 40}, {100000, 25}, {150000, 10}, {200000, 5}
}

special.levelup_reward_exp21 = {
    {2, 9}, {3, 14}, {4, 39}, {6, 24}, {8, 9}, {10, 4}
}

special.levelup_reward_exp31 = {
    {2, 10}, {2.5, 15}, {3, 40}, {3.5, 25}, {4, 8}, {4.5, 5}, {5, 2}
}
special.levelup_reward_exp41 = {
    {2, 10}, {2.5, 15}, {3, 40}, {3.5, 25}, {4, 8}, {4.5, 5}, {5, 2}
}
special.levelup_reward_exp51 = {
    {1, 10}, {1.5, 15}, {2, 40}, {2.5, 25}, {3, 8}, {3.5, 5}, {4, 2}
}
special.levelup_reward_exp84 = {
    {5, 9}, {6, 14}, {7, 39}, {8, 24}, {9, 9}, {10, 4}
}
...
special.warp_to_pos = {
    ...
    {
        -- A 2178 6272
        {217800, 627200}, -- B 2219 5027
        {221900, 502700}, -- C 3440 5025
        {344000, 502500}
    }, -- ���ѻ�
    {
        -- A 4342 2906
        {434200, 290600}, -- B 3752 1749
        {375200, 174900}, -- C 4918 1736
        {491800, 173600}
    }
}
...
special.devil_tower = {
    -- { 123, 608 },
    {2048 + 126, 6656 + 384}, {2048 + 134, 6656 + 147},
    {2048 + 369, 6656 + 629}, {2048 + 369, 6656 + 401},
    {2048 + 374, 6656 + 167}, {2048 + 579, 6656 + 616},
    {2048 + 578, 6656 + 392}, {2048 + 575, 6656 + 148}
}
...
special.lvq_map = {
    { -- "A1" 1
        ...
        {{224, 395}, {137, 894}, {206, 830}, {266, 1067}},
        {{224, 395}, {137, 894}, {206, 830}, {266, 1067}}, {{405, 74}},
        {{405, 74}}, {{405, 74}}, {{405, 74}}, {{405, 74}}, {{405, 74}},
        {{405, 74}}, {{405, 74}}, {{405, 74}}, {{405, 74}}, {{405, 74}},
        {{405, 74}}, {{405, 74}}, {{405, 74}}, {{405, 74}}, {{405, 74}},
        {{405, 74}}, {{405, 74}}, {{405, 74}}, {{405, 74}}, {{405, 74}},
        {{405, 74}}, {{405, 74}}, {{405, 74}}, {{405, 74}}, {{405, 74}}
    }, { -- "A2" 2
        {}, {{640, 1437}}, {{640, 1437}}, {{640, 1437}}, {{640, 1437}},
        {{640, 1437}}, {{640, 1437}}, {{640, 1437}}, {{640, 1437}},
        {{640, 1437}}, {{640, 1437}}, {{640, 1437}}, {{640, 1437}},
        {{640, 1437}}, {{640, 1437}}, {{640, 1437}}, {{640, 1437}},
        {{640, 1437}}, {{640, 1437}}, {{640, 1437}}, {{640, 1437}},
        ...
        {{640, 1437}}, {{640, 1437}}, {{640, 1437}},
        {
            {244, 1309}, {4567, 1080}, {496, 885}, {798, 975}, {1059, 1099},
            {855, 1351}
        },
        {
            {244, 1309}, {4567, 1080}, {496, 885}, {798, 975}, {1059, 1099},
        ...
        {}, {{948, 804}}, {{948, 804}}, {{948, 804}}, {{948, 804}},
        {{948, 804}}, {{948, 804}}, {{948, 804}}, {{948, 804}}, {{948, 804}},
        {{948, 804}}, {{948, 804}}, {{948, 804}}, {{948, 804}}, {{948, 804}},
        {{948, 804}}, {{948, 804}}, {{948, 804}}, {{948, 804}}, {{948, 804}},
        {{948, 804}}, {{948, 804}}, {{948, 804}}, {{948, 804}},
        ...
        {{200, 277}, {130, 646}, {211, 638}, {291, 851}},
        {{200, 277}, {130, 646}, {211, 638}, {291, 851}}, {{100, 150}},
    ...
    { -- "B1" 21
        ...
        {{866, 822}, {706, 224}, {247, 722}}, {{617, 948}, {353, 221}},
        {{617, 948}, {353, 221}}, {{617, 948}, {353, 221}},
        {{617, 948}, {353, 221}}, {{617, 948}, {353, 221}},
        {{617, 948}, {353, 221}}, {{617, 948}, {353, 221}},

        {{496, 1089}, {890, 1043}}, {{496, 1089}, {890, 1043}},
        {{496, 1089}, {890, 1043}}, {{876, 1127}}, {{876, 1127}}, {{876, 1127}},
        {{876, 1127}}, {{876, 1127}}, {{876, 1127}}, {{876, 1127}},

        {{876, 1127}}, {{876, 1127}}, {{876, 1127}}, {{876, 1127}},
        {{876, 1127}}, {{876, 1127}}, {{876, 1127}}, {{876, 1127}},
        {{876, 1127}}, {{876, 1127}}, {{876, 1127}}, {{876, 1127}},
        {{876, 1127}}, {{908, 87}}, {{908, 87}}, {{908, 87}}, {{908, 87}},
        {{908, 87}}, {{908, 87}}
    }, { -- "B2" 22
        {}, {{95, 819}}, {{95, 819}}, {{95, 819}}, {{95, 819}}, {{95, 819}},
        {{95, 819}}, {{95, 819}}, {{95, 819}}, {{95, 819}}, {{95, 819}},
        {{95, 819}}, {{95, 819}}, {{95, 819}}, {{95, 819}}, {{95, 819}},
        {{95, 819}}, {{95, 819}}, {{95, 819}}, {{95, 819}}, {{95, 819}},
        {{95, 819}}, {{95, 819}}, {{95, 819}}, {{746, 1438}}, {{746, 1438}},
        {{746, 1438}}, {{746, 1438}}, {{746, 1438}}, {{746, 1438}},
        {{746, 1438}}, {{746, 1438}}, {{746, 1438}}, {{746, 1438}},
        {{746, 1438}}, {{746, 1438}}, {{746, 1438}}, {{746, 1438}},
        {{746, 1438}}, {{746, 1438}}, {{746, 1438}}, {{746, 1438}},
        {{746, 1438}}, {{746, 1438}},
        ...
        {{172, 810}, {288, 465}, {475, 841}, {303, 156}, {687, 466}},
        {{172, 810}, {288, 465}, {475, 841}, {303, 156}, {687, 466}}, {
            {787, 235}, {1209, 382}, {1350, 571}, {1240, 852}, {1254, 1126},
            {1078, 1285}, {727, 1360}
        }, {
            {787, 235}, {1209, 382}, {1350, 571}, {1240, 852}, {1254, 1126},
            {1078, 1285}, {727, 1360}
        ...
        {{566, 694}, {349, 574}, {198, 645}},
        {{566, 694}, {349, 574}, {198, 645}},
        {{566, 694}, {349, 574}, {198, 645}}, {{816, 721}, {489, 823}},
        {{816, 721}, {489, 823}}, {{816, 721}, {489, 823}}, {{772, 140}},
        {{772, 140}}, {{772, 140}}, {{772, 140}}, {{772, 140}}, {{772, 140}}
    }, {}, -- 24
    {}, -- 25
    {}, -- 26
    {}, -- 27
    ...
    { -- "C1" 41
        ...
        {{452, 160}, {536, 1034}, {184, 1044}},
        {{452, 160}, {536, 1034}, {184, 1044}}, {{137, 126}}, {{137, 126}},
        {{137, 126}}, {{137, 126}}, {{137, 126}}, {{137, 126}}, {{137, 126}},

        {{137, 126}}, {{137, 126}}, {{137, 126}}, {{137, 126}}, {{137, 126}},
        {{137, 126}}, {{137, 126}}, {{137, 126}}, {{137, 126}}, {{137, 126}},
        {{137, 126}}, {{137, 126}}, {{137, 126}}, {{137, 126}}, {{137, 126}},
        {{137, 126}}, {{137, 126}}, {{137, 126}}, {{137, 126}}
    }, { -- "C2" 42
        {}, {{1409, 139}}, {{1409, 139}}, {{1409, 139}}, {{1409, 139}},
        {{1409, 139}}, {{1409, 139}}, {{1409, 139}}, {{1409, 139}},
        {{1409, 139}}, {{1409, 139}}, {{1409, 139}}, {{1409, 139}},
        {{1409, 139}}, {{1409, 139}}, {{1409, 139}}, {{1409, 139}},
        {{1409, 139}}, {{1409, 139}}, {{1409, 139}}, {{1409, 139}},
        ...
        {{1409, 139}}, {{1409, 139}}, {{1409, 139}},
        {{991, 222}, {1201, 525}, {613, 232}, {970, 751}, {1324, 790}},
        {{991, 222}, {1201, 525}, {613, 232}, {970, 751}, {1324, 790}},
        {{991, 222}, {1201, 525}, {613, 232}, {970, 751}, {1324, 790}},
        {{991, 222}, {1201, 525}, {613, 232}, {970, 751}, {1324, 790}},
        {
            {192, 211}, {247, 600}, {249, 882}, {987, 981}, {1018, 1288},
            {1303, 1174}
        },
        {
            {192, 211}, {247, 600}, {249, 882}, {987, 981}, {1018, 1288},
        ...
        {}, {{901, 151}}, {{901, 151}}, {{901, 151}}, {{901, 151}},
        {{901, 151}}, {{901, 151}}, {{901, 151}}, {{901, 151}}, {{901, 151}},
        {{901, 151}}, {{901, 151}}, {{901, 151}}, {{901, 151}}, {{901, 151}},
        {{901, 151}}, {{901, 151}}, {{901, 151}}, {{901, 151}}, {{901, 151}},
        {{901, 151}}, {{901, 151}}, {{901, 151}}, {{901, 151}},
        {{421, 189}, {167, 353}}, {{421, 189}, {167, 353}},
        {{421, 189}, {167, 353}}, {{421, 189}, {167, 353}},
        {{421, 189}, {167, 353}}, {{679, 459}, {505, 709}},
        {{679, 459}, {505, 709}}, {{679, 459}, {505, 709}},
        {{679, 459}, {505, 709}}, {{679, 459}, {505, 709}},
        {{858, 638}, {234, 596}}, {{858, 638}, {234, 596}},
        {{858, 638}, {234, 596}}, {{858, 638}, {234, 596}},
        {{858, 638}, {234, 596}}, {{858, 638}, {234, 596}},
        {{858, 638}, {234, 596}}, {{635, 856}, {324, 855}},
        {{635, 856}, {324, 855}}, {{635, 856}, {324, 855}}, {{136, 899}},
        {{136, 899}}, {{136, 899}}, {{136, 899}}, {{136, 899}}, {{136, 899}}
    }, {}, -- 44
    {}, -- 45
    {}, -- 46
    {}, -- 47
...
function return_charname_by_id(charid)
    local counter = 11
    repeat counter = counter - 1 until char_name_list[counter]["eid"] == charid
...
function inc_today_count(questname, flag_name, count)
    ...
    if last_day == today then
        pc.setf(questname, today_count_flag,
                pc.getf(questname, today_count_flag) + 1)
    else
...
function drop_gamble_with_flag(drop_flag)
    ...
    if drop_chance < 1 or drop_chance > 100 then drop_chance = 10 end
